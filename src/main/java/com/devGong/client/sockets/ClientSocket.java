...
public class ClientSocket {
    ...
    public byte[] makeLocation(String px, String py) {

        // 점 제거 + 10진수로 변환하기   String -> decimal
        int convertedPx = Integer.parseInt(px.replace(".", ""));  //1272411553
        int convertedPy = Integer.parseInt(py.replace(".", "")); //37315906

        // 16진수로 변환하기   decimal -> hexa
        String first = Integer.toHexString(convertedPx); //4bd775a1
        String second = Integer.toHexString(convertedPy);// 2396542
        System.out.println("[String/first] : " + first);
        System.out.println("[String/second] : " + second);

        //바이트 배열에다가 헥사값 두개를 담아서 그걸 리턴해야함
            ...
            try {
                ...
                if (reply != null) {    // 서버에서 날라온 값이 있을 경우, ACK을 Flag로 가져감.
                    ...
                    try {
                        ...
                        System.out.println("[ACK/NAK Result] : " + Arrays.toString(resultArr));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
